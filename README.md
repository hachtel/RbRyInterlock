...
- There is a BNC header to bring in the signal from the photodiode. 
- To ensure that the board is a single layer we have inserted a ground plane. 
- The original designs used linear voltage regulators to condition +15V down to +5V. These were running too hot, so we have added solder pads 1/2 for +5V/GND. We have found that a standard AC/DC wall adapter works well for this purpose.
